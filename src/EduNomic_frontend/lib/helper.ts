...
import { Member, Proposal } from "declarations/EduNomic_backend_dao/EduNomic_backend_dao.did";
...
export function countProposalVotes(proposal: Proposal) {
  const approves = proposal.votes.filter((vote : any) => vote.yesOrNo).length;
  const rejects = proposal.votes.filter((vote : any) => !vote.yesOrNo).length;
  return {
    approves,
    rejects,
  };
