...
import { Proposal } from 'declarations/EduNomic_backend_dao/EduNomic_backend_dao.did';
...
export async function proposalsLoader() {
  let proposals = await EduNomic_backend_dao.getAllProposal();
  return {
    proposals,
  };
}
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setFieldError }) => {
      try {
        ...
        setIsLoading(true);
        const response = (await EduNomic_backend_dao.createProposal(data as any)) as any;
        setIsLoading(false);
                ...
                <HStack mt={10}>
                  ...
                  <Button
                    onClick={async () => {
                      ...
                      setIsLoading(true);
                      const response = (await EduNomic_backend_dao.voteProposal(
                        BigInt(selectedId),
                        {
