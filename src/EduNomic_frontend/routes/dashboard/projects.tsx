...
import { Project } from 'declarations/EduNomic_backend_dao/EduNomic_backend_dao.did';
...
export default function CoursesPage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setFieldError }) => {
      try {
        setIsLoading(true);
        const response = (await EduNomic_backend_dao.createProject(
          values.name,
          values.github,
        )) as any;
        setIsLoading(false);
        if (response.err) {
          ...
          toast({
            ...
            description: 'You can now see the course in the list.',
            status: 'success',
            duration: 9000,
            isClosable: true,
            position: 'top',
  ...
  useEffect(() => {
    async function fetchProjects() {
      const projects = await EduNomic_backend_dao.getAllProjects();
            ...
            <Box>
              <form onSubmit={formik.handleSubmit}>
                <Stack gap={3} mb={5}>
                  ...
                  >
                    <FormLabel>Course Material URL</FormLabel>
                    <Input
